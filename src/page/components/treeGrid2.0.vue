...
<template>
  <div :style="{width:tableWidth}" class='autoTbale'>
    <table class="table table-bordered" id='hl-tree-table'>
      ...
      <tbody>
        <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" @click="RowClick(item,$event,index)" :class="{'child-tr':item.parent, 'tr-active': index === selectIndex}">
          <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
            ...
            <div v-if="column.type === 'action'">
              <i-button :type="action.type" size="small" @click="buttonClick(item,$event,index,action.text)" v-for='action in (column.actions)' style="margin-right:3px" :key="action.text">{{action.text}}</i-button>
            </div>
...
export default {
  ...
  methods: {
    ...
    // 点击按钮
    buttonClick (data, event, index, text) {
      let result = this.makeData(data)
      this.$emit('on-button-click', result, event, index, text)
